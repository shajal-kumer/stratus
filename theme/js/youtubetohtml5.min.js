...
(YouTubeToHtml5.prototype.defaultOptions = {
	...
	(YouTubeToHtml5.prototype.getAllowedFormats = function () {
		...
		return (
			Array.isArray(this.options.formats)
				...
				? (a = [this.options.formats])
				: "*" === this.options.formats && (a = Object.values(this.itagMap).sort()),
			a
		);
	}),
	(YouTubeToHtml5.prototype.getElements = function (a) {
		...
		return (
			a &&
				(NodeList.prototype.isPrototypeOf(a) || HTMLCollection.prototype.isPrototypeOf(a)
					? (b = a)
	...
	(YouTubeToHtml5.prototype.youtubeDataApiEndpoint = function (a) {
		var b = ~~(33 * Math.random()),
			c = encodeURIComponent(
				"https://www.youtube.com/get_video_info?video_id=".concat(a, "&el=embedded&hl=en_US")
			),
			d = "https://images"
				.concat(b, "-focus-opensocial.googleusercontent.com/gadgets/proxy?container=none&url=")
				.concat(c);
		return this.applyFilters("api.endpoint", d, a, b);
	}),
	...
	(YouTubeToHtml5.prototype.parseYoutubeMeta = function (a) {
		var b = this,
			c = [],
			d = [],
			e = this.parseUriString(a);
		return (
			(e.player_response = JSON.parse(e.player_response)),
			(e.fflags = this.parseUriString(e.fflags)),
						...
						if (c) {
							...
							if (d && Array.isArray(d)) {
								...
								h && "url" in h && h.url && (m.src = h.url),
									...
									m.src && "function" == typeof m.src.toString && m.src.toString().length
										? ((a.src = m.src), m.type && m.type.length && (a.type = m.type))
										: console.warn("YouTubeToHtml5 unable to load video for ID: ".concat(e));
							}
						}
					})
					["finally"](function (c) {
